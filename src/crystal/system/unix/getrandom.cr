...
module Crystal::System::Random
  ...
  private def self.getrandom(buf)
    ...
    while buf.size > 0
      ...
      raise OSError.create("getrandom") if read_bytes == -1
  ...
  # and sets `Crystal::System::Errno.value`.
  #
  ...
  private def self.sys_getrandom(buf : Bytes)
    loop do
      ...
      if read_bytes < 0 && (Crystal::System::Errno.value == OSError::EINTR || Crystal::System::Errno.value == OSError::EAGAIN)
        Fiber.yield
      else
        return read_bytes
      end
